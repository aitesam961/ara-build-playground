...
`define FFLNR(__q, __d, __load, clk_i) \
...
`define FF(__q, __d, __reset_value, clk_i, rst_ni) \
  ...
  `define FFARN(__q, __d, __reset_value, clk_i, rst_ni)     \
  always_ff @(posedge (clk_i) or negedge (rst_ni)) begin    \
    if (!rst_ni) begin                                      \
      __q <= (__reset_value);                               \
    end else begin                                          \
...
//// (__clk: clock input)
//// (__arst_n: asynchronous reset, active-low)
`define FFL(__q, __d, __load, __reset_value, clk_i, rst_ni) \
  always_ff @(posedge (clk_i) or negedge (rst_ni)) begin    \
    if (!rst_ni) begin                                      \
      __q <= (__reset_value);                               \
    end else begin                                          \
  ...
  `define FFLARN(__q, __d, __load, __reset_value, clk_i, rst_ni) \
  always_ff @(posedge (clk_i) or negedge (rst_ni)) begin         \
    if (!rst_ni) begin                                           \
      __q <= (__reset_value);                                    \
    end else begin                                               \
...
// __clk: clock input
// __arst_n: asynchronous reset, active-low
`define FFLARNC(__q, __d, __load, __clear, __reset_value, clk_i, rst_ni) \
    `ifndef NO_SYNOPSYS_FF                                                 \
